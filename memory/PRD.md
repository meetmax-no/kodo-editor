...
## V4.0 Close-out 🎉
V4.0 shipped 2026-04-24. Adds Undo and dynamic field addition.
...
### Considered & removed
- **Field validation** was implemented but rejected after testing. Automatic detection based on field name or value patterns is guesswork — JSON has no type metadata, so validation produced too many false positives ("notat" field with `#ZZZ` flagged as invalid hex). Removed entirely in favor of a clean, predictable editor that trusts the user. Live JSON preview already provides visual confirmation of what will be exported.
